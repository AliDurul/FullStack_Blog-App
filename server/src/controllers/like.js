...
module.exports = {
  create: async (req, res) => {
    ...
    let likes = await Like.findOne({user_id:req.user._id, post_id: req.params.id});

    if(likes){
        await Like.deleteOne({_id:likes._id})
        // likes = await Like.findOne({user_id:req.user._id, post_id: req.params.id});
    }else{
     await Like.create({user_id:req.user._id, post_id: req.params.id})
    }
